...
- [ ] 3.2 Modelar em `schema.prisma` todas as entidades do `design.md` — `User` (com `onboardingCompletedAt DateTime?`), `UserProfile` (com `bodyFatPercent Decimal? @db.Decimal(4, 2)`), `Session` (padrão Auth.js), `Food`, `Meal`, `MealItem`, `NutritionGoalOverride`, `AiInteraction`, `RateLimitHit` — com relações, `@db.Decimal(10, 2)` nos macros, enums, `@unique` e `@index` apropriados; verificar com `pnpm prisma validate`.
...
- [ ] 10.2 Implementar telas `/signup` e `/login` (Client Components com Server Actions internas) com Zod validation espelhada ao backend; verificar com RTL cobrindo feedback de senha fraca e e-mail duplicado (cobre "Cadastro bem-sucedido", "E-mail já cadastrado", "Senha fraca" e "Credenciais inválidas").
- [ ] 10.2.1 Implementar `src/app/(app)/onboarding/page.tsx` como Client Component com React Hook Form + Zod cobrindo, num único formulário, data de nascimento, sexo biológico, altura (cm), peso atual (kg), percentual de gordura corporal (opcional, 3–75), nível de atividade e objetivo; submissão chama `PUT /api/v1/profile` e, no sucesso, faz `router.replace('/dashboard')`; verificar com RTL o fluxo feliz (todos os campos válidos → redirect) e o fluxo com body fat vazio (aceito, redirect ocorre igual).
- [ ] 10.2.2 Implementar `src/middleware.ts` que, para rotas do grupo `(app)/*`, verifica a sessão Auth.js e, quando `session.user.onboardingCompletedAt` é nulo, redireciona para `/onboarding` (exceto quando o próprio pathname já é `/onboarding`); para as rotas `/api/v1/*` fora de `auth` e `profile`, `requireUser()` responde 409 `onboarding_required`; verificar com testes de integração cobrindo "Onboarding é a próxima tela após o cadastro", "Bloqueio de rotas até o onboarding ser concluído" e "Onboarding não é repetido em logins subsequentes".
- [ ] 10.2.3 Adicionar ao `api-client.ts` interceptação de resposta 409 `onboarding_required` para redirecionar o usuário para `/onboarding` no cliente; verificar com RTL simulando uma chamada de API que retorna esse status a partir de uma rota autenticada e assertando o redirect.
- [ ] 10.2.4 Fazer o handler de sucesso do `POST /api/v1/auth/signup` redirecionar (server-side) o cliente para `/onboarding` após criar a conta e autenticar o usuário; verificar com RTL de ponta-a-ponta que após um cadastro bem-sucedido a próxima URL renderizada é `/onboarding` e não `/dashboard`.
